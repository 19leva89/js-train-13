...
function manipulateSearchParams(paramsObj, newUrl) {
	...
	// Перебір кожного параметра пошуку з 'paramsObj' та додавання їх до об'єкту 'searchParams'
...
function createURLWithParams(params, url) {
	...
	for (const prop in params) {
		urlObj.searchParams.append(prop, params[prop]);
	}
...
function modifyURLParameters(url, params) {
	...
	for (const prop in params) {
		if (urlObj.searchParams.get(prop, params[prop])) {
 ...
 * Повертається - Об'єкт, ключі якого відповідають ключам вхідного об'єкта, а значеннями є булеві значення, що вказують на наявність відповідного параметра в URL.
 */
function checkURLParameters(url, params) {
	...
	for (const value of params) {
		result[value] = urlObj.searchParams.has(value);
	}
